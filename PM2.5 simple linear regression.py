...
while ii < 10000:
    for i in range(len(y_all)):
        d_SSR_theta1 += (theta1 * (y_last_all[i]) + theta0 - y_all[i]) * y_last_all[i]
        d_SSR_theta0 += (theta1 * (y_last_all[i]) + theta0 - y_all[i])
...
for i in range(0, 4320, 18):
    test_all.append(test_data[9+i,8])
...
for i in range(240):
    final_output[i] = (int(test_all[i])*theta1 + theta0)
